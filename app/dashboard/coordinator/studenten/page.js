...
function bepaalStatus(student, placements) {
  const pl = placements.find(p => p.student_id === student.id)
  if (!pl || pl.status === 'pending' || pl.status === 'cancelled') return 'geen stage'
  if (pl.status === 'active' || pl.status === 'halfway') {
    const goedgekeurdUren = 0 // todo: uren ophalen
    const doel = pl.hours_required || 320
    const pct = goedgekeurdUren / doel
    if (pct >= 0.9) return 'bijna klaar'
    if (pct >= 0.4) return 'op koers'
    return 'op koers'
  }
  if (pl.status === 'invited') return 'geen stage'
...
export default function StudentenPage() {
  ...
  function exportCSV() {
    ...
    const rows = gefilterd.map(s => {
      ...
      return [s.name, s.email || '', s.klas || '', bepaalStatus(s, placements), pl?.company_name || '—', pl?.supervisor_name || '—']
    })
    const csv = [headers, ...rows].map(r => r.join(',')).join('\n')
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url; a.download = 'studenten.csv'; a.click()
  }
  ...
  return (
    <CoordinatorLayout profile={profile}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.5)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }} onClick={() => setEditModal(null)}>
          <div style={{ background: '#fff', borderRadius: 16, padding: 28, maxWidth: 440, width: '100%' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 4 }}>
              ...
              <button onClick={() => setEditModal(null)} style={{ padding: '12px 20px', background: '#F0EDE8', border: 'none', borderRadius: 10, color: '#5C6B7A', fontWeight: 600, fontSize: 14, cursor: 'pointer' }}>Annuleren</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ padding: '28px 32px' }}>
        ...
        {/* Filters toolbar */}
        <div style={{ background: '#fff', border: '1px solid #E4DDD4', borderRadius: 12, padding: '14px 20px', marginBottom: 16, display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
          {/* Zoek */}
          <div style={{ position: 'relative', flex: 1, minWidth: 180 }}>
            <span style={{ position: 'absolute', left: 11, top: '50%', transform: 'translateY(-50%)', fontSize: 14, pointerEvents: 'none' }}>🔍</span>
            <input type="text" placeholder="Zoek student..." value={zoek} onChange={e => setZoek(e.target.value)}
          ...
          <select value={filterPeriode} onChange={e => setFilterPeriode(e.target.value)}
            style={{ padding: '8px 12px', border: '1.5px solid #E4DDD4', borderRadius: 8, fontFamily: 'Inter,sans-serif', fontSize: 13, color: '#1A2633', background: '#fff', outline: 'none' }}>
        ...
        <div style={{ background: '#fff', borderRadius: 12, overflow: 'hidden', border: '1px solid #E4DDD4' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 2fr 1fr 80px', padding: '11px 20px', background: '#F7F3EE', borderBottom: '1px solid #E4DDD4', fontSize: 12, fontWeight: 700, color: '#5C6B7A', textTransform: 'uppercase', letterSpacing: '.05em' }}>
            ...
            return (
              <div key={student.id} style={{ borderBottom: '1px solid #EDE8E2' }}>
                <div
                  ...
                  style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 2fr 1fr 80px', padding: '13px 20px', alignItems: 'center', cursor: 'pointer', background: isOpen ? '#F7F3EE' : '#fff' }}
                >
                  ...
                  <div style={{ fontSize: 13 }}>
                    {pl?.company_name ? (
                      <div>
                        <div style={{ fontWeight: 600 }}>{pl.company_name}</div>
                        <div style={{ fontSize: 12, color: '#5C6B7A' }}>{pl.supervisor_name || '—'}</div>
                      </div>
                    ) : <span style={{ color: '#5C6B7A' }}>—</span>}
                  </div>
                  <div style={{ fontSize: 13, color: '#5C6B7A' }}>0 / {pl?.hours_required || 320} u</div>
                  <div style={{ display: 'flex', gap: 6 }} onClick={e => e.stopPropagation()}>
                    <button onClick={() => openEdit(student)} style={{ width: 30, height: 30, borderRadius: 8, border: '1.5px solid #E4DDD4', background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: 14 }}>✏️</button>
                    {student.email && <button onClick={() => window.location.href = `mailto:${student.email}`} style={{ width: 30, height: 30, borderRadius: 8, border: '1.5px solid #E4DDD4', background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: 14 }}>📧</button>}
                  </div>
                </div>
                  ...
                  <div style={{ padding: '0 20px 16px', background: '#FAFAF8', borderTop: '1px solid #EDE8E2' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 14 }}>
                      ...
                      <div style={{ background: '#EAF1F6', borderRadius: 10, padding: 14 }}>
                        ...
                        {pl.company_address && <div style={{ fontSize: 12, color: '#5C6B7A' }}>📍 {pl.company_address}, {pl.company_postcode} {pl.company_city}</div>}
        ...
        <div style={{ marginTop: 12, fontSize: 13, color: '#5C6B7A' }}>{gefilterd.length} van {studenten.length} studenten</div>
      </div>
    </CoordinatorLayout>
  )
}
